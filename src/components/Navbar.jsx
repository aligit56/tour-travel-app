...
function Navbar({ links, onDestinations, onJournal, onWhy, onHome }) {
    ...
    <header className="absolute inset-x-0 top-0 z-20 text-white">
      <nav className="mx-auto mt-3 flex max-w-7xl items-center justify-between rounded-full border border-white/20 bg-[#102e2c]/35 px-5 py-3.5 shadow-xl shadow-black/10 backdrop-blur-md sm:px-7 lg:px-8">
        ...
        <div className="hidden items-center gap-9 md:flex">
          ...
          <a className="rounded-full bg-[#f8f5ef] px-5 py-3 text-sm font-bold text-[#18312f] shadow-lg shadow-black/10 transition-transform hover:-translate-y-0.5" href="#tours">Plan a trip</a>
        </div>
      ...
      {isOpen && <div className="border-t border-white/20 bg-[#18312f]/95 px-5 py-5 md:hidden"><div className="flex flex-col gap-5">{links.map((link) => <a className={`nav-link relative w-fit font-medium ${activeLink === link.href ? 'text-white' : 'text-white/75'}`} href={link.href} key={link.label} onClick={(event) => { if (link.label === 'Destinations') { event.preventDefault(); onDestinations() } if (link.label === 'Journal') { event.preventDefault(); onJournal() } if (link.label === 'Why Roamwell') { event.preventDefault(); onWhy() } handleLinkClick(link.href) }}>{link.label}</a>)}<a className="w-fit rounded-full bg-[#f8f5ef] px-5 py-2.5 text-sm font-bold text-[#18312f]" href="#tours">Plan a trip</a></div></div>}
    </header>
  )
}
